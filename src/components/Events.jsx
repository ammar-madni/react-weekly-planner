...
{/* <div className="p-4 space-y-4 bg-gray-100 rounded-md editing-mode">
          ...
          <div className="flex justify-between align-center space-x-2 editing-mode">
              ...
              onClick={() => {
                if (editable) {
                  ...
                  const newState = produce(eventData, draft => {
                    draft[year][week][format(day, "EEE d")][hour][index] = textareaRef.current.value
                  })
            ...
            <button
              onClick={() => {
                const newState = produce(eventData, draft => { (draft[year][week][format(day, "EEE d")][hour]).splice(index, 1) })
                setEventData(newState)
              }}
              type="button"
